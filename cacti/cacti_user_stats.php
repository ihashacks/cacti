...
$user_logins_valid = db_fetch_cell("SELECT count(*) as user_logins FROM `user_log` WHERE UNIX_TIMESTAMP(`time`) BETWEEN (UNIX_TIMESTAMP() - $poller_interval - MOD(UNIX_TIMESTAMP(), $poller_interval)) AND (UNIX_TIMESTAMP()-MOD(UNIX_TIMESTAMP(), $poller_interval)) AND `user_id` > 0 AND `result` = 1");
$user_logins_invalid = db_fetch_cell("SELECT count(*) as user_logins FROM `user_log` WHERE UNIX_TIMESTAMP(`time`) BETWEEN (UNIX_TIMESTAMP() - $poller_interval - MOD(UNIX_TIMESTAMP(), $poller_interval)) AND (UNIX_TIMESTAMP()-MOD(UNIX_TIMESTAMP(), $poller_interval)) AND `user_id` = 0 AND `result` = 0");
...
$poller_interval_end = time() - time()%$poller_interval;
$poller_interval_begin = $poller_interval_end - $poller_interval;
...
/* first off check if we are allowed to read the session file. Then we are only interested in sessions of authenticated Cacti users */
...
Sessions with a MTIME higher than the end of the poller interval have to be counted too or we won't see real active users.
*/
if($mtime >= $poller_interval_begin) {
...
if($mtime >= ($poller_interval_begin - $session_maxlifetime) & $mtime < $poller_interval_begin) {
...
if($mtime < ($poller_interval_begin - $session_maxlifetime)) {
...
print "valid:$user_logins_valid invalid:$user_logins_invalid active:$session_counter_active sleeping:$session_counter_sleeping garbage:$session_counter_garbage uactive:$user_counter_active usleeping:$user_counter_sleeping";
